...
class Trainer():
    def __init__(self, sess, policy, policy_optimizer, gp_controller, logger,
                 pool, n_samples=2000000, batch_size=1000, alpha=0.5, topk=5,
    ...
    def run_one_step(self, override=None):
        ...
        # NEW: update topk rewards
        for p, rew in zip(programs, r):
            if len(self.topk_r) < self.topk:
                self.topk_p.append(p)
                self.topk_r.append(rew)
            else:
                min_idx = np.argmin(self.topk_r)
                if self.topk_r[min_idx] < rew:
                    self.topk_r[min_idx] = rew
                    self.topk_p[min_idx] = p
